...
input = """[1518-10-03 00:47] falls asleep
...
for line in input2:
    ...
    date = split_line[0].replace('[1518-', '')
    #print(date)
    #print(date)
    #print(date)
    #print(date)
    #print(date)
    d = datetime.datetime.strptime(date,"%m-%d %M:%S")

    date_actions.append((d, split_line[1]))

#print(date_actions)

is_asleep = False
guard_on_job = -1
guards_length = [0] * 8000
prev_time = -1
guard_most = []
...
for action_time in date_actions:
    date = action_time[0]
    ...
    if "begins shift" in action:
        guard_id = int(action.split(' ')[2].replace('#', ''))
        guard_on_job = guard_id
    ...
    if "wakes up" in action:
        is_asleep = False
        while prev_time != date:
            guards_length[guard_on_job] += 1
            #print(prev_time)
            if prev_time.second in guard_most[guard_on_job]:
                guard_most[guard_on_job][prev_time.second] += 1
            else:
                guard_most[guard_on_job][prev_time.second] = 1
            prev_time = prev_time + datetime.timedelta(0,1)

        #if guard_on_job == 1471:
            #if int(action_time[0].minute) in guard_most:
                #guard_most[int(action_time[0].minute)] += 1
            #lse:
                #guard_most[int(action_time[0].minute)] = 1

guard_id = -1
max_len = -1
for index in range(len(guards_length)):
    if guards_length[index] > max_len:
        max_len = guards_length[index]
        guard_id = index
...
max_len = -1
guard = -1
for index in range(len(guard_most)):
    for min, freq in guard_most[index].items():
        #print(guard_most[index])
        print(freq)
        if freq > max_len:
            max_len = freq
            minute = min
